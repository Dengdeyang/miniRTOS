...
//---------------------------------------------------------------------//
uint32 LED_Read(uint32 pos,void *buffer,uint32 size)
{		
	*((uint8 *)buffer) = GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_12);
	return size;
}

//---------------------------------------------------------------------//
uint32 LED_Write(uint32 pos,const void *buffer,uint32 size)
{		
	GPIO_WriteBit(GPIOB,GPIO_Pin_12,(BitAction)(*((uint8 *)buffer)));
	return size;
}

//---------------------------------------------------------------------//
void LED_Control(uint32 cmd, void *arg)
{	
	GPIO_InitTypeDef GPIO_InitStructure;
